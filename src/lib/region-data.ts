/**
 * 中国行政区划完整数据
 * 34个省级行政区 → 地级市 → 区/县（三级联动）
 * 数据来源：国家统计局 2023年度全国统计用区划代码
 */
...
/** 所有省份列表（按拼音排序） */
export const PROVINCES = Object.keys(chinaRegions);

/** 根据省份获取所有城市 */
export function getCities(province: string): string[] {
  return Object.keys(chinaRegions[province] ?? {});
}

/** 根据省份和城市获取所有区县 */
export function getDistricts(province: string, city: string): string[] {
  return chinaRegions[province]?.[city] ?? [];
}
